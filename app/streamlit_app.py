...
from wbkc.signal import detect_peak, detrend, find_roi, sg_smooth
...
live_time_s = st.sidebar.number_input(
    "Live time (s)", min_value=10, max_value=24 * 3600, value=900, step=10
)

st.sidebar.subheader("Calibration")
cps_per_TBK = st.sidebar.number_input(
    "cps_per_TBK", min_value=1e-6, value=100.0, step=1.0, format="%.6f"
)
bg_cps = st.sidebar.number_input(
    "Background cps in ROI", min_value=0.0, value=0.1, step=0.05
)
attn_mean = st.sidebar.number_input(
    "Attenuation mean", min_value=0.01, value=1.0, step=0.01
)
attn_rel_sigma = st.sidebar.number_input(
    "Attenuation relative sigma", min_value=0.0, value=0.05, step=0.01, format="%.4f"
)

n_mc = st.sidebar.number_input(
    "MC samples", min_value=500, max_value=100000, value=5000, step=500
)
...
def gen_synth(
    n=3001, keV_max=3000.0, true_tbk=2.5, cps_per_TBK=100.0, live_time_s=900, seed=42
):
...
res = simulate(
    ...
    live_time_s,
    calib=dict(
        cps_per_TBK=cps_per_TBK,
        bg_cps=bg_cps,
        attn_mean=attn_mean,
        attn_rel_sigma=attn_rel_sigma,
    ),
    n_mc=int(n_mc),
...
with st.expander("Precision vs. time"):
    chart_df = pd.DataFrame(
        {"time_s": t_grid, "rel_precision": pred_hw95 / max(res["tbk_mean"], 1e-12)}
    )
...
with st.expander("Download TBK samples (CSV)"):
    ...
    st.download_button(
        "Download", data=csv, file_name="tbk_samples.csv", mime="text/csv"
    )
